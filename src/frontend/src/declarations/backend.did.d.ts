/* eslint-disable */

// @ts-nocheck

// This file was automatically generated by @caffeinelabs/bindgen@0.1.0.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import type { ActorMethod } from '@icp-sdk/core/agent';
import type { IDL } from '@icp-sdk/core/candid';
import type { Principal } from '@icp-sdk/core/principal';

export interface Course {
  'id' : bigint,
  'title' : string,
  'instructor' : string,
  'description' : string,
  'language' : string,
  'schedule' : string,
  'enrollmentOpen' : boolean,
}
export interface ShoppingItem {
  'productName' : string,
  'currency' : string,
  'quantity' : bigint,
  'priceInCents' : bigint,
  'productDescription' : string,
}
export interface StripeConfiguration {
  'allowedCountries' : Array<string>,
  'secretKey' : string,
}
export type StripeSessionStatus = {
    'completed' : { 'userPrincipal' : [] | [string], 'response' : string }
  } |
  { 'failed' : { 'error' : string } };
export interface TransformationInput {
  'context' : Uint8Array,
  'response' : http_request_result,
}
export interface TransformationOutput {
  'status' : bigint,
  'body' : Uint8Array,
  'headers' : Array<http_header>,
}
export type UserRole = { 'admin' : null } |
  { 'user' : null } |
  { 'guest' : null };
export interface http_header { 'value' : string, 'name' : string }
export interface http_request_result {
  'status' : bigint,
  'body' : Uint8Array,
  'headers' : Array<http_header>,
}
export interface _SERVICE {
  '_initializeAccessControlWithSecret' : ActorMethod<[string], undefined>,
  'addCourse' : ActorMethod<[Course], undefined>,
  'assignCallerUserRole' : ActorMethod<[Principal, UserRole], undefined>,
  'createAnnouncement' : ActorMethod<
    [
      {
        'title' : string,
        'body' : string,
        'date' : bigint,
        'language' : string,
      },
    ],
    undefined
  >,
  'createCheckoutSession' : ActorMethod<
    [Array<ShoppingItem>, string, string],
    string
  >,
  'deleteAnnouncement' : ActorMethod<[bigint], undefined>,
  'getAllApplications' : ActorMethod<
    [],
    Array<
      {
        'name' : string,
        'email' : string,
        'message' : string,
        'timestamp' : bigint,
        'phone' : string,
        'course' : string,
      }
    >
  >,
  'getAllContactMessages' : ActorMethod<
    [],
    Array<
      {
        'subject' : string,
        'name' : string,
        'email' : string,
        'message' : string,
        'timestamp' : bigint,
        'phone' : string,
      }
    >
  >,
  'getAllDonations' : ActorMethod<
    [],
    Array<
      {
        'donorName' : string,
        'email' : string,
        'currency' : string,
        'timestamp' : bigint,
        'amount' : bigint,
        'purpose' : string,
      }
    >
  >,
  'getAllEnrollments' : ActorMethod<
    [],
    Array<
      {
        'studentName' : string,
        'plan' : string,
        'email' : string,
        'currency' : string,
        'timestamp' : bigint,
        'courseId' : bigint,
      }
    >
  >,
  'getAnnouncements' : ActorMethod<
    [],
    Array<
      {
        'title' : string,
        'body' : string,
        'date' : bigint,
        'language' : string,
      }
    >
  >,
  'getCallerUserProfile' : ActorMethod<
    [],
    [] | [{ 'name' : string, 'email' : string, 'phone' : string }]
  >,
  'getCallerUserRole' : ActorMethod<[], UserRole>,
  'getCourseLiveStatus' : ActorMethod<[], Array<[bigint, boolean]>>,
  'getCourseLiveStatusFor' : ActorMethod<[bigint], boolean>,
  'getCourses' : ActorMethod<[], Array<Course>>,
  'getStripeConfiguration' : ActorMethod<[], StripeConfiguration>,
  'getStripeSessionStatus' : ActorMethod<[string], StripeSessionStatus>,
  'getUserProfile' : ActorMethod<
    [Principal],
    [] | [{ 'name' : string, 'email' : string, 'phone' : string }]
  >,
  'isCallerAdmin' : ActorMethod<[], boolean>,
  'isStripeConfigured' : ActorMethod<[], boolean>,
  'recordDonation' : ActorMethod<
    [
      {
        'donorName' : string,
        'email' : string,
        'currency' : string,
        'timestamp' : bigint,
        'amount' : bigint,
        'purpose' : string,
      },
    ],
    undefined
  >,
  'recordEnrollment' : ActorMethod<
    [bigint, string, string, string, string],
    undefined
  >,
  'saveCallerUserProfile' : ActorMethod<
    [{ 'name' : string, 'email' : string, 'phone' : string }],
    undefined
  >,
  'setCourseLive' : ActorMethod<[bigint, boolean], undefined>,
  'setStripeConfiguration' : ActorMethod<[StripeConfiguration], undefined>,
  'submitApplication' : ActorMethod<
    [string, string, string, string, string],
    undefined
  >,
  'submitContactMessage' : ActorMethod<
    [string, string, string, string, string],
    undefined
  >,
  'transform' : ActorMethod<[TransformationInput], TransformationOutput>,
  'updateAnnouncement' : ActorMethod<
    [
      bigint,
      {
        'title' : string,
        'body' : string,
        'date' : bigint,
        'language' : string,
      },
    ],
    undefined
  >,
  'updateCourse' : ActorMethod<[Course], undefined>,
}
export declare const idlService: IDL.ServiceClass;
export declare const idlInitArgs: IDL.Type[];
export declare const idlFactory: IDL.InterfaceFactory;
export declare const init: (args: { IDL: typeof IDL }) => IDL.Type[];